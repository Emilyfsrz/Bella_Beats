...
DECLARE
 MORNING_START INT64 DEFAULT 6;
DECLARE
 MORNING_END INT64 DEFAULT 12;
DECLARE
 AFTERNOON_END INT64 DEFAULT 18;
DECLARE
 EVENING_END INT64 DEFAULT 21;
...
SELECT
  ...
  COUNT(table_name)
FROM
  adroit-approach-414021.Bella.INFORMATION_SCHEMA.COLUMNS
GROUP BY
 1;
...
SELECT 
  table_name,
  SUM(CASE
WHEN data_type IN ("TIMESTAMP", "DATETIME", "DATE") THEN 1
   ELSE
   0
 END
   ) AS has_time_info
...
FROM
 adroit-approach-414021.Bella.daily_activities
LIMIT 
5;
...
FROM adroit-approach-414021.Bella.INFORMATION_SCHEMA.COLUMNS 
...
FROM 
  adroit-approach-414021.Bella.daily_activities A
LEFT JOIN
  adroit-approach-414021.Bella.daily_calories C
ON
A.Id = C.Id
AND A.ActivityDate=C. ActivityDay
AND A.Calories = C.Calories
LEFT JOIN 
 adroit-approach-414021.Bella.daily_intensities I
 ON 
 A.Id = I.Id
 ...
 LEFT JOIN adroit-approach-414021.Bella.daily_steps S
 ON 
 A.Id = S.Id
 AND A.ActivityDate=S.ActivityDay
LEFT JOIN
  adroit-approach-414021.Bella.Sleep_day S1
ON
  A.Id = S1.Id
  AND A.ActivityDate = CAST(S1.SleepDay AS DATE);
...
MAX(DATE(date)) AS sleep_end,
 ...
 FROM
   adroit-approach-414021.Bella.minute_sleep
 WHERE
   value=1
 GROUP BY
   1,
...
WITH user_dow_summary AS (
  SELECT
    ...
    CASE
     ...
     WHEN TIME(ActivityHour) >= TIME(EVENING_END, 0, 0) OR TIME(TIMESTAMP_TRUNC(ActivityHour, MINUTE)) <= TIME(MORNING_START, 0, 0) THEN "Night"
     ELSE "ERROR"
    END AS time_of_day,
  ...
  FROM
    adroit-approach-414021.Bella.hourly_intensities
  GROUP BY
